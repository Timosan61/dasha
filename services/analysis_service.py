...
ANALYSIS_PROMPT = """Проанализируй данные Instagram-подписчиков из одного сегмента.

ВАЖНО: Анализируй ТОЛЬКО на основе реальных данных ниже. НЕ придумывай боли или проблемы, которых нет в данных.
...
Задача: определи для этого сегмента СТРОГО на основе данных:

1. **Название сегмента** (2-4 слова, описывающие кто эти люди)
2. **Портрет представителя** (кто они по профессии/роли, чем занимаются)
3. **Что их объединяет** (общие интересы, деятельность, ценности — только из данных)
4. **Их потребности/боли** (если видны в данных; если не видны — напиши "не определяется из данных")
5. **Чего хотят достичь** (если видно из постов/bio)
6. **Типичное высказывание** (цитата или парафраз из bio/постов)
7. **Темы контента** (о чём пишут, что публикуют)
